...
async function initColonne2() {
  ...
  async function renderFileContent(filename, preselectItems = []) {
    ...
    entries.forEach(([item, nuances]) => {
      ...
      if (isActive) {
        line.classList.add("bg-emerald-600");
      }
      line.style.cssText = `padding: 8px; margin-bottom: 6px; border-radius: 6px; cursor: pointer; display: flex; align-items: center; gap: 10px; transition: 0.2s; background: ${isActive ? "#10b981" : "#1e293b"}; color: #e5e7eb; border: 1px solid ${isActive ? "#059669" : "#334155"};`;
      ...
      line.onclick = async () => {
        ...
        line.dataset.active = nextActive ? "1" : "0";
        if (nextActive) {
          line.classList.add("bg-emerald-600");
          line.style.background = "#10b981";
          line.style.borderColor = "#059669";
          titleSpan.style.color = "#ffffff";
          window.selectedItems.add(item);
        } else {
          line.classList.remove("bg-emerald-600");
          line.style.background = "#1e293b";
          line.style.borderColor = "#334155";
          titleSpan.style.color = "#60a5fa";
