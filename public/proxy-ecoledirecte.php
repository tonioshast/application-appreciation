...
// --- TRANSFORMATION DES PHOTOS EN BASE64 ---
$decodedResponse = json_decode($response, true);

if ($httpCode === 200 && isset($decodedResponse['data']['eleves']) && is_array($decodedResponse['data']['eleves'])) {
    foreach ($decodedResponse['data']['eleves'] as &$item) {
        // Attention : la structure réelle contient une sous-clé 'eleve'
        if (isset($item['eleve']) && !empty($item['eleve']['photo'])) {
            $photoUrl = $item['eleve']['photo'];
            
            // Si l'URL commence par '//', on lui ajoute 'https:'
            if (strpos($photoUrl, '//') === 0) {
                $photoUrl = 'https:' . $photoUrl;
            }
            
            // Appel cURL interne pour récupérer la photo de l'élève
            $chPhoto = curl_init($photoUrl);
            curl_setopt($chPhoto, CURLOPT_RETURNTRANSFER, true);
            curl_setopt($chPhoto, CURLOPT_SSL_VERIFYPEER, false);
            curl_setopt($chPhoto, CURLOPT_FOLLOWLOCATION, true);
            curl_setopt($chPhoto, CURLOPT_REFERER, 'https://www.ecoledirecte.com/');
            
            $imageData = curl_exec($chPhoto);
            $photoHttpCode = curl_getinfo($chPhoto, CURLINFO_HTTP_CODE);
            curl_close($chPhoto);

            if ($photoHttpCode === 200 && $imageData) {
                $finfo = new finfo(FILEINFO_MIME_TYPE);
                $mimeType = $finfo->buffer($imageData);
                // On remplace l'URL par la chaîne data en Base64
                $item['eleve']['photo'] = 'data:' . $mimeType . ';base64,' . base64_encode($imageData);
            } else {
                $item['eleve']['photo'] = ''; // Vide en cas d'échec
            }
        }
    }
    // On ré-encode la réponse modifiée pour l'envoyer au front-end
    $response = json_encode($decodedResponse, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE);
}
// ------------------------------------------
